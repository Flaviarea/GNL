...
{
    int str_len;
    char *next_str; // to store the small strings read by buff of create_list in one bigger string
...
void    append(t_list **list, char *buf)
{
    ...
    if (last_node == NULL) // if the list is empty
        *list = new_node; // the new node becomes the first one  
    else
        last_node->next = new_node; // the new node will be linked to the last one 
...
{
    int char_read;
    char *buf; // temporary buf with BUFFER_SIZE byted read inside

    while(!found_newline(*list)) // if there is no \n 
    {
        buf = malloc(BUFFER_SIZE + 1);
        if ( buf == NULL)
            return ;
...
{
    static t_list *list;
    char    *next_line; // to store the return value of get_line
